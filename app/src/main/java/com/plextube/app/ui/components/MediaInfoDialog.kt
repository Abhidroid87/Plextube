...
fun MediaInfoDialog(
    ...
    var mediaInfo by remember { mutableStateOf<com.plextube.app.innertube.models.MediaInfo?>(null) }
    ...
    LaunchedEffect(track, video) {
        ...
        if (videoId != null) {
            mediaInfo =
                com.plextube.app.data.newmusic.InnertubeMusicService
                    .getMediaInfo(videoId)
            resolvedDurationSeconds = mediaInfo?.durationSeconds?.takeIf { it > 0 }
                ?: resolvedDurationSeconds
                ?: com.plextube.app.data.music.YouTubeMusicService
