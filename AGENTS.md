...
Plextube is a media player that runs for hours at a time. Jank, dropped frames, playback stutter,
...
6. **Event-driven over polling.** Prefer player callbacks, Plextube emissions, and
...
10. **The shared pools are finite.** `PerformanceDispatcher.networkIO` is a fixed 4–16 thread
    ...
    in flight.
11. **Plextube lifecycle**: `stateIn(WhileSubscribed(5_000))` on UI-facing flows is load-bearing —
...
13. Sustained heat while the app is open = per-frame work; drain with the screen off = CPU/network
    ...
    (c) check `adb shell dumpsys gfxinfo io.github.abhidroid87.plextube` for continuous frame production
...
Plextube uses Hilt, but some legacy app-owned classes are still reached through static/companion
...
11. DI and SOLID are maintainability/testability tools, not automatic performance improvements.
    ...
    expanded singleton lifetimes, retained `Context`/Activity references, duplicate Plextube
...
13. Minimum validation for a non-player DI migration is `ktlintCheck`,
    ...
    or background Plextube on a device/emulator, including configuration change and an error path. A
...
- `shared/` primitives with no domain meaning take the **`Plextube`** prefix: `PlextubeFilterChip`,
  `PlextubeSearchField`, `PlextubeEmptyState`, `PlextubeLoadingIndicator`.
...
`ui/components/shared/` — the app already has `PlextubeEmptyState`, `PlextubeErrorState`, `MediaRow`,
`MediaThumbnail`, `MediaBadges`, `PlextubeSearchField`, `ShimmerLoading` and `FastScrollbar`.
...
2. After changing the cold-start path — `MainActivity.onCreate`, `PlextubeApp`, app-level DI graph,
